...
<h3>SOOP Fast Repetition Rate Fluorometry</h3>
...
${feature.time_coverage_start.value} to<BR>${feature.time_coverage_end.value}<BR>
  ...
  <#if feature.callsign.value == "VLHJ           " >
        <img class="spaced" src="${my.baseurlImage}/southern_surveyor.jpg" alt="${feature.vessel_name.value}  ${feature.callsign.value}" /><BR>
  </#if>
  
  <#if feature.callsign.value == "FHZI           " > <#-- spaces needed due to weird SQL Server stuff -->
        <img class="spaced" src="${my.baseurlImage}/l_astrolabe.jpg" alt="${feature.vessel_name.value}  ${feature.callsign.value}" /><BR>
  </#if>
...
<a href="http://mest.imas.utas.edu.au/geonetwork/srv/en/metadata.show?uuid=${feature.dataset_uuid.value}"  target="_blank"  rel="external" class="h3" 
title="http://mest.imas.utas.edu.au/geonetwork/srv/en/metadata.show?uuid=${feature.dataset_uuid.value}">Data metadata record</a><BR>
